...
In~\cite{MP} it is proved that
$$ N_l (p,q)= R_\Gamma \times_{D_\Gamma} T_\Gamma \approx (S^{p-1}\times S^{q-1}) \times_{\mathbb{Z}_2\times \mathbb{Z}_2} S^1 \times S^1,
$$
where the actions on the right side are antipodal involutions, and on the left side they are given by
\begin{align*}
&\varphi_1: (u_1,\dots ,u_m)\rightarrow(-u_1,\dots,-u_k,-u_{k+1},\dots,-u_p,u_{p+1},\dots,u_m),\\
&\varphi_2: (u_1,\dots ,u_m)\rightarrow(-u_1,\dots,-u_k,u_{k+1},\dots,u_p,-u_{p+1},\dots,-u_m).
\end{align*}

\noindent So the topological type of $N$ is defined by three numbers $p$,$q$ and $l$ where $p+q=m$ and $0 \leq l \leq p$. 
...
Consider the $n$-dimensional submanifold $$\tilde{N} = \tilde{\pi} ^{-1}(U_{\Gamma \Delta}) \cong (R_{\Gamma \Delta}/D_\Gamma) \times_{D_\Delta} T_\Delta$$ of the manifold $ Z_{\Gamma \Delta}/T_\Gamma  \subset V_\Gamma $. Similarly to Theorems~\ref{theoA} and~\ref{theoB}, we get:
\begin{main}
